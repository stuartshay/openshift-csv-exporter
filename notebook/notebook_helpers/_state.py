...
# Populated by ``bootstrap()`` — maps cluster_name -> cluster_server. Used as
# a recovery path when a query selects only ``cluster_name`` so we can still
# derive env / friendly_name and enable the Cluster filter on every grid.
